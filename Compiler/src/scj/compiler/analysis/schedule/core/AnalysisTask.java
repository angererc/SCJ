	...
	private AnalysisStepResult<Instance, TV, SM> analyze(AnalysisTaskResolver<Instance, TV, SM> resolver, FormalParameterConstraints myParamConstraints) {
		...
		while(scheduleSites.hasNext()) {
			...
			while(formalParameters.hasNext()) {
				...
				case happensBefore:				
					myResult.formalParameterResult.addTasksNotOrderedAfter(myFormalParam, possibleTargetTasks);
					break;
				case happensAfter:
					myResult.formalParameterResult.addTasksNotOrderedBefore(myFormalParam, possibleTargetTasks);
					break;
				case ordered:
					//?!? nothing to do here?
					break;
				...
				myResult.formalParameterResult.addTasksNotOrderedBefore(myFormalParam, unorderedChildren.tasksNotOrderedBefore);
				myResult.formalParameterResult.addTasksNotOrderedAfter(myFormalParam, unorderedChildren.tasksNotOrderedAfter);
			}
