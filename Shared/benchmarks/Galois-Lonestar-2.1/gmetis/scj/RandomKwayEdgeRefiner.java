...
public class RandomKwayEdgeRefiner {
	...
	public void scjTask_refine(Task<Void> now, Task<?> later, MetisGraph metisGraph) throws Exception {
		Task<Void> end = new Task<Void>();
		this.scjTask_refineEnd(end);
		end.hb(later);
		
		Task<Void> startIteration = new Task<Void>();
		this.scjTask_refineIteration(startIteration, end, 0, -42, metisGraph);
		startIteration.hb(end);
	}
	
	public void scjTask_refineEnd(Task<Void> now) {
		
	}

	public void scjTask_refineIteration(Task<Void> now, Task<?> later, Integer passParam, Integer previousOldCut, final MetisGraph metisGraph) throws Exception {
		...
		if(previousOldCut == oldcut)
			return;
		
		Task<Void> nextPass = new Task<Void>();
		this.scjTask_refineIteration(nextPass, later, pass++, oldcut, metisGraph);
		nextPass.hb(later);
		...
		GaloisSCJComputation<GNode<MetisNode>> computation = new UnorderedGaloisSCJComputation<GNode<MetisNode>>(boundary, Priority.first(RandomPermutation.class));
