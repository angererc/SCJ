...
File: DirectedListEdgeGraph.java 
...
public class DirectedListEdgeGraph<NodeData, EdgeData> implements EdgeGraph<NodeData, EdgeData> {

  List<EdgeGraphNode> nodes;

  public DirectedListEdgeGraph() {
    nodes = Collections.synchronizedList(new ArrayList<EdgeGraphNode>());
  }
  ...
  public boolean removeNeighbor(Node<NodeData> src, Node<NodeData> dest) {
    ...
    // if gsrc.removeOutEdge(gdest), then return gdest.removeInEdge(gsrc)
  ...
  protected class ListMap<Key, Val> {
    ...
    protected boolean containsKey(Key n) {
      return keys.contains(n);
    }

    protected boolean put(Key k, Val v) {
      if (keys.contains(k))
        return false;
      keys.add(k);
      vals.add(v);
    ...
    protected boolean remove(Key n) {
      int idx = keys.indexOf(n);
      if (idx < 0)
        return false;
      keys.remove(idx);
      vals.remove(idx);
    ...
    protected Val get(Key n) {
      return vals.get(keys.indexOf(n));
    }

    protected Collection<Val> values() {
      return vals;
    }

    protected Collection<Key> keySet() {
      return keys;
    }
  }

  protected class EdgeGraphNode implements Node<NodeData> {
    protected ListMap<EdgeGraphNode, GraphEdge> inEdges;
    protected ListMap<EdgeGraphNode, GraphEdge> outEdges;
    ...
    EdgeGraphNode(NodeData d) {
      inEdges = new ListMap<EdgeGraphNode, GraphEdge>();
      outEdges = new ListMap<EdgeGraphNode, GraphEdge>();
      data = d;
    }
