...
/**
 * Encapsulates functionality needed to support deterministic replay of Galois
 * iterators.
 * 
 * 
 */
public abstract class ReplayFeature {
	...
	protected final void setNextRid(Iteration it, int iterationId,
			Replayable obj) {
		if (it != null) {
			int index = getCounterIndex(iterationId);
			if (first[index] == 0) {
	...
	protected final void setNextRid(Replayable obj) {
		// Depend on ctx.createReplayable() to set rid during parallel
		// execution,
		// which in turn should call setNextRid(int,Object) or
		// setNextRid(Iteration,int,Object)
	...
	public abstract <T> void registerCallback(long rid,
			LambdaVoid<ForeachContext<T>> callback);
	...
	private static final class Rids {
		...
		private static long makeRid(int maxtIterations, int iterationId,
				int counter) {
	...
	/**
	 * Log that stores active elements and callbacks for deterministic replay.
	 * 
	 * 
	 */
	static final class Log implements Serializable {
		...
		/**
		 * Creates a new log file capable of storing at most the given capacity
		 * of entries.
		 * 
		 * @param capacity
