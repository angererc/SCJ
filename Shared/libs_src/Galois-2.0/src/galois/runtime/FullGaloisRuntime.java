 ...
 *
 */
public final class FullGaloisRuntime extends GaloisRuntime {
	...
	private FullGaloisRuntime(int numThreads, boolean useParameter, boolean useSerial, ReplayFeature.Type replayType,
	...
	public static FullGaloisRuntime getFullRuntime() {
		if(instance == null) {
			// Use default serial Runtime
			initialize(1, false, true, ReplayFeature.Type.NO, false, false);
		}
		return (FullGaloisRuntime)instance;
	}

	/**
	 * Called by the testing framework to reset the runtime.
	 */
	public static void initialize(int numThreads, boolean useParameter, boolean useSerial,
			ReplayFeature.Type replayType, boolean moreStats, boolean ignoreUserFlags) {
		if (instance != null) {
			instance.invalidate();
		}

		instance = new FullGaloisRuntime(numThreads, useParameter, useSerial, replayType, moreStats, ignoreUserFlags);
	}
	...
	throws ExecutionException {
		getFullRuntime().runBody(initial, body, priority);
	}
	...
	throws ExecutionException {
		getFullRuntime().runOrderedBody(initial, body, priority);
	}
	...
	public static <T> void foreach(Mappable<T> initial, LambdaVoid<T> body) throws ExecutionException {
		getFullRuntime().runBody(initial, body);
	}
	...
	public static <T, A1> void foreach(Mappable<T> initial, Lambda2Void<T, A1> body, A1 arg1) throws ExecutionException {
		getFullRuntime().runBody(initial, body, arg1);
	}
	...
	throws ExecutionException {
		getFullRuntime().runBody(initial, body, arg1, arg2);
	}
		...
		if (replayType == ReplayFeature.Type.PLAYBACK) {
			...
			if (type == ExecutorType.ORDERED) {
				...
				stats = pushContextAndCall(ex, new Callable<IterationStatistics>() {
					@Override
					public IterationStatistics call() throws Exception {
						System.out.println("GaloisRuntime: calling executor " + ex);
						return ex.call(body, wl);
					}
				});
			}
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < numRuns; i++) {
			...
			Features.initialize(getFullRuntime().getMaxIterations(), replayType);
