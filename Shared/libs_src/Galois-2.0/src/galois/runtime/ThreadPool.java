...
class ThreadPool {
  ...
  public void callAll(List<? extends Callable<?>> callables) throws InterruptedException, ExecutionException {
	  System.out.println("ThreadPool.callAll");
    this.callables = callables;
    start.release(numThreads); //signal to workers that there is work
  ...
  private class Worker implements Runnable {
    ...
    public Worker(int id) {
    	System.out.println("ThreadPool$Worker created " + id);
      this.id = id;
    }

    private void __stackSamplerRecordMe() throws Exception {
    	System.out.println("\tThreadPool$Worker calling callable " + id);
      callables.get(id).call();
    }

    @Override
    public void run() {
    	System.out.println("ThreadPool$Worker starts running " + id);
      while (!shutdown) {
        try {
          start.acquire();
          if (!shutdown) {
